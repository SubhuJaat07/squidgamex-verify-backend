/**********************************************************************
 * 🚀 SQUID GAME X - ULTIMATE MEGA BOT
 * Features: Verification, Invite Tracker, Gift System, Auto Mod,
 * Maintenance Mode, Ban List, Active User Pagination.
 **********************************************************************/
...
// Global Variables
let MAINTENANCE_MODE = false; // Maintenance Status

// --- 🗄️ SUPABASE SETUP 🗄️ ---
const supabase = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);

// --- 🌐 EXPRESS SERVER (API & Keep Alive) 🌐 ---
const app = express();
app.use(cors());
...
// 📄 ACTIVE USERS PAGINATION GENERATOR
async function generateActiveUsersPayload(page) {
    ...
    const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`active_prev_${page}`).setLabel('◀ Previous').setStyle(ButtonStyle.Secondary).setDisabled(page === 1),
        new ButtonBuilder().setCustomId(`active_next_${page}`).setLabel('Next ▶').setStyle(ButtonStyle.Primary).setDisabled(page >= totalPages)
    );

    return { embeds: [embed], components: [row] };
}

// 🚫 BAN LIST PAGINATION GENERATOR
async function generateBanListPayload(page) {
    const limit = 10;
    const offset = (page - 1) * limit;

    const { data: bannedUsers, count } = await supabase.from("verifications")
        .select("code, hwid, discord_id", { count: 'exact' })
        .eq("is_banned", true)
        .range(offset, offset + limit - 1);

    if (!bannedUsers || bannedUsers.length === 0) {
        return { embeds: [new EmbedBuilder().setColor(0x00FF00).setTitle("✅ No Banned Users")], components: [] };
    }

    const totalPages = Math.ceil((count || 0) / limit);
    const embed = new EmbedBuilder()
        .setColor(0xFF0000)
        .setTitle(`🚫 Banned Users (Page ${page}/${totalPages})`)
        .setDescription(`**Total Bans:** ${count}`)
        .setTimestamp();

    let desc = "";
    for (const [i, u] of bannedUsers.entries()) {
        desc += `**${offset + i + 1}. Code:** \`${u.code}\`\n   **HWID:** \`...${u.hwid.slice(-8)}\`\n   **User:** ${u.discord_id ? `<@${u.discord_id}>` : "Unknown"}\n\n`;
    }
    embed.setDescription(desc);

    const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`banlist_prev_${page}`).setLabel('◀ Previous').setStyle(ButtonStyle.Secondary).setDisabled(page === 1),
        new ButtonBuilder().setCustomId(`banlist_next_${page}`).setLabel('Next ▶').setStyle(ButtonStyle.Primary).setDisabled(page >= totalPages)
    );
...
client.on("interactionCreate", async interaction => {
    ...
    if (interaction.isButton()) {
        ...
        if (interaction.customId.startsWith('banlist_')) {
            const [_, direction, currentPage] = interaction.customId.split('_');
            let newPage = parseInt(currentPage) + (direction === 'next' ? 1 : -1);
            await interaction.deferUpdate();
            const payload = await generateBanListPayload(newPage);
            await interaction.editReply(payload);
            return;
        }
    }

    // SYNC TRACKER BUTTONS
    ...
    if (commandName === "admin") {
        ...
        if (sub === "banlist") {
             ...
             const payload = await generateBanListPayload(1);
        ...
        if (sub === "stats") {
             ...
             const statsEmbed = new EmbedBuilder().setColor(0x00FFFF).setTitle("📊 Server Statistics")
                .addFields(
                    ...
                    { name: "Maintenance Mode", value: MAINTENANCE_MODE ? "🔴 ON" : "🟢 OFF", inline: true }
                );
        ...
        if (sub === "generate") {
             ...
             return interaction.reply({ content: `🎁 **Gift Key Generated:** \`${code}\`\n⏳ Duration: ${durationStr}`, ephemeral: true });
        }

        if (sub === "maintenance") {
             const status = interaction.options.getString("status");
             MAINTENANCE_MODE = status === 'on';
             return interaction.reply({ content: `🚧 Maintenance Mode is now **${status.toUpperCase()}**`, ephemeral: true });
        }
        
        // Announce & Purge moved here
        if (sub === "announce") {
    ...
    if (commandName === "activeusers") {
        ...
        await interaction.deferReply(); 
        const payload = await generateActiveUsersPayload(1); // Page 1
        return interaction.editReply(payload);
    }

    if (commandName === "redeem") {
        await interaction.deferReply({ ephemeral: true });
        if (MAINTENANCE_MODE) return interaction.editReply("🚧 System is in Maintenance.");
        ...
        if (!user) return interaction.editReply("❌ You don't have a Verification session yet. Please use `/verify` first.");
        ...
        return interaction.editReply(`✅ **Redeemed Successfully!**\nTime Added: ${gift.duration}`);
    }

    // --- STANDARD COMMANDS (Verify, Invites etc.) ---
    if (commandName === "verify") {
        if (MAINTENANCE_MODE) return interaction.reply({ content: "🚧 Maintenance Break. Try later.", ephemeral: true });
        await interaction.deferReply();
    ...
    // Tracker & Config Handlers (Same as before)
